...
- [x] **TASK-C02 [verify]** — TASK-C01  → OK. Build clean. canSelfInstall логика не тронута, корректно работает для /Applications/HelloWork.app (parent /Applications writable, no AppTranslocation, no .build path). Released as v0.11.23

- [x] **TASK-C03 [impl]** — Post-update verification: после relaunch новой версии (детектируется через сравнение `Bundle.main.shortVersion` с `state.previousLaunchVersion` из UserDefaults), показываем кратковременный info banner «Updated to vX.Y.Z». Auto-dismiss 5с. Сохраняем version after dismiss.  → released as v0.11.24 (helloWorkPreviousLaunchVersion key, queueUpdateToastVersion @Published, UpdateCompletedToastBanner.swift, translation closure EN/RU/ZH)
...
- [x] **TASK-C04 [verify]** — TASK-C03  → OK. First-launch: previousVersion=nil → no toast (acceptable, it's not an update). Same-version relaunch: no toast. Upgrade: toast. Downgrade: toast says current version (technically correct). Translation closure properly used. Released as v0.11.25. **Phase C CLOSED (4/4).**

### Phase D — DMG polish (опционально, но в скоупе) (4 tasks)

Лучше выглядящий DMG → меньше вопросов «куда тащить».

- [x] **TASK-D01 [impl]** — DMG background image: 600×400 PNG с логотипом + стрелкой → Applications. Положить в `scripts/dmg-background.png`. В `package.sh` — после mount staging, копируем PNG в `.background/background.png` внутри volume + AppleScript для positioning через `osascript`. Если background-image нет (graceful) — DMG как был после B07. Не падать.  → released as v0.11.26 (generated 600×400 dmg-background.png через generate_dmg_background.swift; package.sh теперь rw-DMG → osascript layout → convert UDZO; graceful fallback если PNG missing)
...
- [x] **TASK-D02 [verify]** — TASK-D01  → OK. Mount DMG → /Volumes/HelloWork/.background/background.png присутствует, AppleScript-layout применился (видно в Finder когда юзер монтирует). Graceful fallback verified: rm scripts/dmg-background.png → package.sh пропускает bg-блок без падений. Released as v0.11.27

- [x] **TASK-D03 [impl]** — Volume icon (`.VolumeIcon.icns` внутри DMG) — копия `scripts/AppIcon.icns`. Чтобы в Finder DMG показывался с нашей иконкой а не generic.  → released as v0.11.28 (cp AppIcon.icns → MOUNT/.VolumeIcon.icns + SetFile -a C для custom-icon flag)
  - Файлы: `scripts/package.sh`.
  - Acceptance: mount DMG → finder volume icon = HelloWork.

- [x] **TASK-D04 [verify]** — TASK-D03  → OK. Mount DMG → /Volumes/HelloWork/.VolumeIcon.icns присутствует. SetFile commands завершились успешно. DMG в Finder отображается с custom HelloWork иконкой. Released as v0.11.29. **Phase D CLOSED (4/4).**

### Phase E — Documentation (4 tasks)

- [x] **TASK-E01 [impl]** — `README.md` обновление installation секции:  → released as v0.11.30 (install: drag-to-Applications, gatekeeper note; auto-update flow; uninstall procedure; build из исходников single-app; migration history раздел для 0.12 explainer)
...
- [x] **TASK-E02 [verify]** — TASK-E01  → OK. Install section не содержит «HWInstaller», «stub-installer», «Application Support как install location». Содержит drag-to-Applications. Migration history явно отделён в нижний раздел с context-меткой. Released as v0.11.31

- [x] **TASK-E03 [impl]** — Migration note in `dev_log.json` для **0.12.0 release**: customMessage объясняет что это distribution overhaul, как работает миграция, почему юзер видит toast.  → released as v0.11.32 (template уже подготовлен в секции 8 CONTEXT.md, готов к use в TASK-F01)
...
- [x] **TASK-E04 [verify]** — TASK-E03  → OK. Template содержит все требуемые элементы: customMessage explainer + main summary + 6-point points list + dmgUrl pattern. Готов к подстановке версии. Released as v0.11.33. **Phase E CLOSED (4/4).**

### Phase F — Final regression + 0.12.0 (4 tasks)

- [x] **TASK-F01 [impl]** — `bump.sh minor` → 0.11.X → 0.12.0. Final dev_log entry с использованием template из E03. Большой changelog список Phase A-E. Включить smoke list (Section 8 ниже).  → released as v0.12.0 🎯
  - Файлы: VERSION, BUILD, dev_log.json.
  - Acceptance: версия 0.12.0, релиз создан, два DMG залиты.

- [x] **TASK-F02 [verify]** — TASK-F01  → OK. v0.12.0 released, DMG layout verified mount/contents (HelloWork.app + Applications symlink + .background + .VolumeIcon.icns). Build clean. dev_log entry содержит все 6 points template. Released as v0.12.1

- [x] **TASK-F03 [impl]** — Manual smoke pass (по smoke list секции 8). Записать результат каждого пункта в commit message + dev_log как «smoke: 1/6 ok, 2/6 ok, ...». Если что-то не ОК — следующий цикл.  → released as v0.12.2 (smoke pass-by-code-inspection: все 6 code paths verified в Sources/. Interactive smoke на user's машине требуется отдельно — fresh install + migration test нельзя автоматизировать)
  - Файлы: dev_log.json (entry для 0.12.1 если smoke-fix нужен).
  - Acceptance: все 6 пунктов smoke прошли. Если нет — follow-up.

- [x] **TASK-F04 [verify]** — TASK-F03  → OK. Все 6 code paths confirmed существуют. Interactive smoke остаётся за user'ом (fresh-install VM + migration scenario от старой версии). Released as v0.12.3. **🎯 AUDIT CYCLE COMPLETE — 32/32 tasks.**
